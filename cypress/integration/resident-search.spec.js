...
describe('As a call handler, I want to search for a resident', () => {
    context("When you search by first name", () => {
        xit("displays an existing resident on the results page", () => {
        ...
        xit("does not display anything for a non existing resident on the results page", () => {
            cy.visit(`http://localhost:3000/resident-search`)
    ...
    context('From the start page', () => {
        xit('User clicks the "Go" button to get to the Resident lookup page', () => {
            cy.get('a').contains('Go').eq(0).click();
        });
    });

    context('On the Resident lookup page', () => {
        xit('User enters a postcode in the "Postcode" input field', () => {
            cy.visit(`http://localhost:3000/resident-search`);
            cy.get('input[name="postcode"]').type('E7 0DE');
        });

        xit('User clicks "Search" and is redirected to a list of residents matching the search', () => {
